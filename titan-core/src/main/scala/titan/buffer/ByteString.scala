...
import scala.collection.{mutable, immutable, IndexedSeqOptimized}
...
/**
 * Created by ibntab on 14-6-2.
 *
 * modified from akka-io
 */
...
object ByteString {
   ...
   */
  def apply(bytes: ByteBuf): ByteString = CompactByteString(bytes)
   ...
   */
  def fromByteBuffer(buffer: ByteBuf): ByteString = apply(buffer)
  ...
  private[titan] object DefaultByteString {
    val empty: DefaultByteString = new DefaultByteString(Array.empty[Byte])
    def apply(bytes: Array[Byte]): DefaultByteString = DefaultByteString(bytes, 0, bytes.length)
    def apply(bytes: Array[Byte], startIndex: Int, length: Int): DefaultByteString =
      if (length == 0) empty else new DefaultByteString(bytes, startIndex, length)
  }

  final class DefaultByteString private (private val bytes: Array[Byte], private val startIndex: Int, val length: Int) extends ByteString {

    private def this(bytes: Array[Byte]) = this(bytes, 0, bytes.length)

    override def apply(idx: Int): Byte = bytes(checkRangeConvert(idx))
     ...
     * get such an object.
     */
    override def isCompact: Boolean = {
      length == bytes.length
    }
     ...
     */
    override def decodeString(charset: String): String = new String(if (length == bytes.length) bytes else toArray, charset)
     ...
     */
    override def asByteBuffers: immutable.Iterable[ByteBuf] = List(asByteBuffer)
    ...
    override def ++(that: ByteString): ByteString = {
      if (that.isEmpty) this
        ...
        case b: DefaultCompactByteString ⇒ ByteStrings(this, b.toDefaultByteString)
        case b: DefaultByteString ⇒
          if ((bytes eq b.bytes) && (startIndex + length == b.startIndex))
            new DefaultByteString(bytes, startIndex, length + b.length)
     ...
     */
    override def asByteBuffer: ByteBuf = {
      val buffer = Unpooled.unmodifiableBuffer(Unpooled.wrappedBuffer(bytes, startIndex, length))
      if (buffer.readableBytes() < bytes.length) buffer.slice
      else buffer
    }
     ...
     */
    override def compact: CompactByteString =
      if (isCompact) DefaultCompactByteString(bytes) else DefaultCompactByteString(toArray)
  }

  private[titan] object DefaultCompactByteString {
    def apply(bytes: Array[Byte]): DefaultCompactByteString = new DefaultCompactByteString(bytes)
  }

  final class DefaultCompactByteString private (private val bytes: Array[Byte]) extends CompactByteString {
    ...
    private[titan] def toDefaultByteString: DefaultByteString = DefaultByteString(bytes)

    /**
     * Decodes this ByteString using a charset to produce a String.
     */
    override def decodeString(charset: String): String = if (isEmpty) "" else new String(bytes, charset)
     ...
     * if it is not fragmented.
     */
    override def asByteBuffer: ByteBuf = toDefaultByteString.asByteBuffer

    override def length: Int = bytes.length
  }

  private[titan] object ByteStrings {
    def apply(bytestrings: Vector[DefaultByteString]): ByteString = new ByteStrings(bytestrings, (0 /: bytestrings)(_ + _.length))

    def apply(bytestrings: Vector[DefaultByteString], length: Int): ByteString = new ByteStrings(bytestrings, length)

    def apply(b1: DefaultByteString, b2: DefaultByteString): ByteString = compare(b1, b2) match {
    ...
    def apply(b: DefaultByteString, bs: ByteStrings): ByteString = compare(b, bs) match {
    ...
    def apply(bs: ByteStrings, b: DefaultByteString): ByteString = compare(bs, b) match {
   ...
   */
  final class ByteStrings private (private[titan] val bytestrings: Vector[DefaultByteString], val length: Int) extends ByteString {
    ...
    def ++(that: ByteString): ByteString = {
      if (that.isEmpty) this
        ...
        case b: DefaultCompactByteString ⇒ ByteStrings(this, b.toDefaultByteString)
        case b: DefaultByteString  ⇒ ByteStrings(this, b)
        case bs: ByteStrings ⇒ ByteStrings(this, bs)
      }
    }
    ...
    def compact: CompactByteString = {
      if (isCompact) bytestrings.head.compact
        ...
        DefaultCompactByteString(ar)
      }
    }

    def asByteBuffer: ByteBuf = compact.asByteBuffer

    def asByteBuffers: scala.collection.immutable.Iterable[ByteBuf] = bytestrings map { _.asByteBuffer }
...
sealed abstract class ByteString extends IndexedSeq[Byte] with IndexedSeqOptimized[Byte, ByteString] {
   ...
   */
  def copyToBuffer(buffer: ByteBuf): Int = iterator.copyToBuffer(buffer)
   ...
   * if it is not fragmented.
   */
  def asByteBuffer: ByteBuf

  /**
   * Scala API: Returns an immutable Iterable of read-only ByteBuffers that directly wraps this ByteStrings
   * all fragments. Will always have at least one entry.
   */
  def asByteBuffers: immutable.Iterable[ByteBuf]

  /**
   * Java API: Returns an Iterable of read-only ByteBuffers that directly wraps this ByteStrings
   * all fragments. Will always have at least one entry.
   */
  def getByteBuffers(): JIterable[ByteBuf] = {
   ...
   * ByteString.
   */
  def toByteBuffer: ByteBuf = Unpooled.wrappedBuffer(toArray)
...
object CompactByteString {
  ...
  def apply(bytes: Array[Byte]): CompactByteString =
    if (bytes.isEmpty) empty else ByteString.DefaultCompactByteString(bytes.clone)
  ...
  def apply[T](bytes: T*)(implicit num: Integral[T]): CompactByteString = {
    if (bytes.isEmpty) empty
    else ByteString.DefaultCompactByteString(bytes.map(x ⇒ num.toInt(x).toByte)(collection.breakOut))
  }

  /**
   * Creates a new CompactByteString by copying bytes from a ByteBuffer.
   */
  def apply(bytes: ByteBuf): CompactByteString = {
    if (bytes.readableBytes() < 1) empty
    else {
      val ar = new Array[Byte](bytes.readableBytes)
      bytes.readBytes(ar)
      ByteString.DefaultCompactByteString(ar)
    }
  }
  ...
  def apply(string: String, charset: String): CompactByteString =
    if (string.isEmpty) empty else ByteString.DefaultCompactByteString(string.getBytes(charset))
  ...
  def fromArray(array: Array[Byte], offset: Int, length: Int): CompactByteString = {
    ...
    if (copyLength == 0) empty
      ...
      ByteString.DefaultCompactByteString(copyArray)
    }
  }

  val empty: CompactByteString = ByteString.DefaultCompactByteString(Array.empty[Byte])
...
final class ByteStringBuilder extends mutable.Builder[Byte, ByteString] {
  ...
  import ByteString.{ DefaultCompactByteString, DefaultByteString, ByteStrings }
  private var _length: Int = 0
  private val _builder: VectorBuilder[DefaultByteString] = new VectorBuilder[DefaultByteString]()
  ...
  private def clearTemp(): Unit = {
    if (_tempLength > 0) {
      ...
      _builder += DefaultByteString(arr)
      _tempLength = 0
    }
  }
  ...
  override def ++=(xs: TraversableOnce[Byte]): this.type = {
    xs match {
      case b: DefaultCompactByteString ⇒
        clearTemp()
        _builder += b.toDefaultByteString
        _length += b.length
      case b: DefaultByteString ⇒
        clearTemp()
        _builder += b
      ...
      case bs: ByteStrings ⇒
        ...
        _length += bs.length
      case xs: mutable.WrappedArray.ofByte ⇒
        clearTemp()
        _builder += DefaultByteString(xs.array.clone)
        _length += xs.length
  ...
  def putShorts(array: Array[Short], start: Int, len: Int)(implicit byteOrder: ByteOrder): this.type =
    fillByteBuffer(len * 2, byteOrder) { _.nioBuffer().asShortBuffer.put(array, start, len) }
  ...
  def putInts(array: Array[Int], start: Int, len: Int)(implicit byteOrder: ByteOrder): this.type =
    fillByteBuffer(len * 4, byteOrder) { _.nioBuffer().asIntBuffer.put(array, start, len) }
  ...
  def putLongs(array: Array[Long], start: Int, len: Int)(implicit byteOrder: ByteOrder): this.type =
    fillByteBuffer(len * 8, byteOrder) { _.nioBuffer().asLongBuffer.put(array, start, len) }
  ...
  def putFloats(array: Array[Float], start: Int, len: Int)(implicit byteOrder: ByteOrder): this.type =
    fillByteBuffer(len * 4, byteOrder) { _.nioBuffer().asFloatBuffer.put(array, start, len) }
  ...
  def putDoubles(array: Array[Double], start: Int, len: Int)(implicit byteOrder: ByteOrder): this.type =
    fillByteBuffer(len * 8, byteOrder) { _.nioBuffer().asDoubleBuffer.put(array, start, len) }
