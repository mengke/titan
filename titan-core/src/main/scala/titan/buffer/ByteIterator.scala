...
object ByteIterator {
  ...
  object ByteArrayIterator {
    ...
    protected[titan] def apply(array: Array[Byte]): ByteArrayIterator =
      new ByteArrayIterator(array, 0, array.length)

    protected[titan] def apply(array: Array[Byte], from: Int, until: Int): ByteArrayIterator =
  ...
  class ByteArrayIterator private (private var array: Array[Byte], private var from: Int, private var until: Int) extends ByteIterator {
     ...
     * accordingly.
     */
    override def asInputStream: java.io.InputStream = new java.io.InputStream {
    ...
    private def wrappedByteBuffer: ByteBuf = Unpooled.unmodifiableBuffer(Unpooled.wrappedBuffer(array, from, len))
     ...
     */
    override def getBytes(xs: Array[Byte], offset: Int, n: Int): this.type = {
      if (n <= this.len) {
     ...
     */
    override def getShorts(xs: Array[Short], offset: Int, n: Int)(implicit byteOrder: ByteOrder): this.type = {
      wrappedByteBuffer.order(byteOrder).nioBuffer().asShortBuffer.get(xs, offset, n)
      drop(2 * n)
    }
     ...
     */
    override def getInts(xs: Array[Int], offset: Int, n: Int)(implicit byteOrder: ByteOrder): this.type = {
      wrappedByteBuffer.order(byteOrder).nioBuffer().asIntBuffer.get(xs, offset, n)
      drop(4 * n)
    }
     ...
     */
    override def getLongs(xs: Array[Long], offset: Int, n: Int)(implicit byteOrder: ByteOrder): this.type = {
      wrappedByteBuffer.order(byteOrder).nioBuffer().asLongBuffer.get(xs, offset, n)
      drop(8 * n)
    }
     ...
     */
    override def getDoubles(xs: Array[Double], offset: Int, n: Int)(implicit byteOrder: ByteOrder): this.type = {
      wrappedByteBuffer.order(byteOrder).nioBuffer().asDoubleBuffer.get(xs, offset, n)
      drop(8 * n)
    }
     ...
     */
    override def getFloats(xs: Array[Float], offset: Int, n: Int)(implicit byteOrder: ByteOrder): this.type = {
      wrappedByteBuffer.order(byteOrder).nioBuffer().asFloatBuffer.get(xs, offset, n)
      drop(4 * n)
    }
    ...
    override def toByteString: ByteString = {
      val result =
        if ((from == 0) && (until == array.length)) ByteString.DefaultCompactByteString(array)
        else ByteString.DefaultByteString(array, from, len)
      clear()
      result
    }
     ...
     */
    override def copyToBuffer(buffer: ByteBuf): Int = {
      val copyLength = math.min(buffer.writableBytes(), len)
      if (copyLength > 0) {
        buffer.writeBytes(array, from, copyLength)
        drop(copyLength)
      }
      copyLength
    }
  ...
  object MultiByteArrayIterator {
    ...
    protected[titan] def apply(iterators: LinearSeq[ByteArrayIterator]): MultiByteArrayIterator =
  ...
  class MultiByteArrayIterator private (private var iterators: LinearSeq[ByteArrayIterator]) extends ByteIterator {
    ...
    private[titan] def ++:(that: ByteArrayIterator): this.type = {
    ...
    def copyToBuffer(buffer: ByteBuf): Int = {
